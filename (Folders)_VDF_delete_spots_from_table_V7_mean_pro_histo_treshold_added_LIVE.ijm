...
// create VDF and, Table with removed spots, Projection
	...
	setBatchMode("exit and display"); 
		Table.save(File.getParent(directory)+File.separator+"Results_All_denoised["+Prominence_treshold+"]_"+File.getNameWithoutExtension(directory)+".csv");
			close("Results_All_denoised");
		selectWindow("Map"); 
			saveAs("Tiff", File.getParent(directory)+File.separator+"VDF_denoised_"+File.getNameWithoutExtension(directory)+".tif");
			close();
		selectWindow("Projection");
			saveAs("Tiff", File.getParent(directory)+File.separator+"All_"+File.getNameWithoutExtension(directory)+".tif");
			close();
			close("*");
	}
}